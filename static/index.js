...
function getFlights(){
    var settings = {
        "async": true,
        "crossDomain": true,
        "url": "https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/autosuggest/v1.0/UK/GBP/en-GB/?query=kenya",
        "method": "GET",
        "headers": {
            "x-rapidapi-host": "skyscanner-skyscanner-flight-search-v1.p.rapidapi.com",
            "x-rapidapi-key": "e98f1131d1msh102be82e4993467p135b92jsnab60a6f16013"
        }
    }

    $.ajax(settings).done(function (response) {
        console.log(JSON.parse(JSON.stringify(response['Places'])));
        if(response){
            var tt = JSON.parse(JSON.stringify(response['Places']))
            // console.log();
            var op = tt.map(function(item) {
    ...
    function processRequest(e) {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            $("#cityselect > option").each(function() {
                if(String(this.value.toLowerCase()) === String(response['address']['country_code']).toLowerCase() ){
...
function myplacesList() {
    ...
    var $seleinput = $('#cityselect')
    for (x of countryListAllIsoData) {
        $('#cityselect').append('<option value="'+x['code']+'">'+ x["name"] +'</option>')
        // console.log(x);
    }
    getCoordintes();
}
